...
class CartPoleAgent():
    ...
    def get_max_Q_model_action(self, state, head):
        ...
        #Let's try to balance the action selection between mean and worst case
        balanced_Q = 0.5*(Q_predictions[0, head, :, 0] + np.mean(Q_predictions[0, head, :, :], axis=-1))
        #shape (actions,)
        action = np.argmax(balanced_Q)
        #shape flat single value
        best_Q_value = np.mean(Q_predictions[0, head, action, :])
    ...
    def get_max_Q_model_actions(self, states, heads):
        ...
        #Let's try to balance the action selection between mean and worst case
        balanced_Q = 0.5*(Q_predictions[np.arange(len(Q_predictions)), heads, :, 0] + np.mean(Q_predictions[np.arange(len(Q_predictions)), heads, :, :], axis=-1))
        #shape (examples, actions)
        action = np.argmax(balanced_Q, axis=-1)
        #shape (examples,)
        best_Q_values = np.mean(
            Q_predictions[np.arange(len(Q_predictions)), heads, action, :],
            axis=-1
