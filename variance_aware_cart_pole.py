...
class CartPoleAgent():
    def __init__(self, flat_dim, action_dim, n_heads=10, alpha=0.5):
    ...
    def get_max_Q_model_action(self, state, head):
        model_predictions = self.Q_model.predict(np.array([state]), verbose=0)
        #shape = (examples, heads, actions, mu_or_log_var)
        Q_predictions = model_predictions[:, :, :, 0] #take only the average of the predictions for now
        log_var_predictions = model_predictions[:, :, :, 1]

        #select the action that maximizes a variance adjusted Q evaluation
        action = np.argmax(Q_predictions[0, head, :] - self.alpha*np.exp(log_var_predictions)[0, head, :])
        best_Q_value= Q_predictions[0, head, action] #return the Q value to match
        return best_Q_value, action

    def get_max_Q_model_actions(self, states, heads):
        model_predictions = self.Q_model.predict(states, verbose=0)
        Q_predictions = model_predictions[:, :, :, 0] # take only the average of the predictions for now
        log_var_predictions = model_predictions[:, :, :, 1]

        action = np.argmax(
            Q_predictions[np.arange(len(Q_predictions)), heads, :]
            -self.alpha*np.exp(log_var_predictions)[np.arange(len(Q_predictions)), heads, :],
            axis=1
        )
        ...
        best_Q_values = Q_predictions[np.arange(len(Q_predictions)), heads, action]
