...
def nll_loss(y_true, y_pred):
    #y_true shape = (examples, actions)
    #y_pred = (examples, actions, mu or log_var)
    mu = y_pred[:, :, 0] #shape = (examples, actions)
    log_var = y_pred[:, :, 1]+1e-8
    var = tf.math.exp(log_var)
    nll = 0.5*log_var+(1/(2*var))*(y_true-mu)**2 #shape=(examples, actions)
...
def make_traditional_action_model(state_dim, action_dim):
    ...
    mu_reshape=keras.layers.Reshape(target_shape=(action_dim, 1))(mu)
    
    log_var = keras.layers.Dense(
        action_dim,
        activation='softplus'
    )(dense_2)
    log_var_reshape=keras.layers.Reshape(target_shape=(action_dim, 1))(log_var)
    output = keras.layers.Concatenate()([mu_reshape, log_var_reshape])
    #shape = (examples, action, mu or log_var)
...
class CartPoleAgent():
    def __init__(self, flat_dim, action_dim):
        ...
        self.Q_prime_model = make_traditional_action_model(
            state_dim=flat_dim,
            action_dim=action_dim
            #n_peaks=self.n_peaks,
        )
    ...
    def get_max_Q_traditional_model_action(self, state):
        ...
        Q_predictions = Q_predictions[:, :, 0] #take only the average of the prediction for now
    ...
    def get_min_Q_prime_traditional_model_value(self, state, action):
        ...
        Q_prime_predictions = self.Q_prime_model.predict(np.array([state]), verbose=0)
        Q_prime_predictions = Q_prime_predictions[:, :, 0] # take only the average of the prediciton for now
        
        Q_prime_value = Q_prime_predictions[0, action]
    ...
    def get_min_Q_traditional_prime_model_values(self, states, actions):
        ...
        Q_prime_predictions = self.Q_prime_model.predict(states, verbose=0)
        Q_prime_predictions = Q_prime_predictions[:, :, 0] # take only the average of the prediction for now
        Q_prime_values = Q_prime_predictions[np.arange(len(Q_prime_predictions)), actions]
...
#this average only version of this silliness trained
#slowly, but it trained.
def main(args):
    ...
    agent.Q_prime_model.set_weights(agent.Q_model.get_weights())
    ...
    for episode_num in track(range(total_episodes), console=console):
        ...
        while not done:
            ...
            if epsilon_choice < epsilon:
                ...
                Q_value = agent.Q_model.predict(np.array([obs]), verbose=0)[0, action, 0] #take the average of the prediction for now
                epsilon_choices+=1
            else:
            ...
            if total_steps >= replay_buffer_warmup_period:
                ...
                final_Q_targets=agent.Q_model.predict(flat_states, verbose=0)[:, :, 0] #Take hte average of the predictions for now
