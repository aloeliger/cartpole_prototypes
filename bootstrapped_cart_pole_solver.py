...
def make_bootstrap_action_model(state_dim, action_dim, n_heads):
    ...
    #number of heads = number of actions * number of heads
    flat_output = keras.layers.Dense(
        action_dim*n_heads,
        activation='linear',
    )(dense_2)
    #shape (head, action)
    output = keras.layers.Reshape(
        target_shape=(n_heads,action_dim)
    )(flat_output)
...
class CartPoleAgent():
    def __init__(self, flat_dim, action_dim, n_heads=10):
        ...
        self.Q_prime_model = make_bootstrap_action_model(
            state_dim=flat_dim,
            action_dim=action_dim,
            n_heads=n_heads
        )
    ...
    def get_Q_prime_model_value(self, state, action, head):
        ...
        Q_prime_predictions = self.Q_prime_model.predict(
            np.array([state]),
            verbose=0,
        )
        Q_prime_value = Q_prime_predictions[0, head, action]
    ...
    def get_Q_prime_model_values(self, states, actions, heads):
        ...
        Q_prime_values = Q_prime_predictions[np.arange(len(Q_prime_predictions)), heads, actions]
...
def main(args):
    ...
    for episode_num in track(range(total_episodes), console = console):
        ...
        while not done:
            ...
            head_mask = nprng.binomial(1, p=0.5, size=(n_heads,))
            ...
            if total_steps >= replay_buffer_warmup_period:
                ...
                final_Q_targets[update_mask] = np.tile(target_array.reshape((128,1,1)), (1,10,2))[update_mask]
